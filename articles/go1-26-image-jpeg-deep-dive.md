...
いただいた金銭は技術書、もしくはマネーフォワード 京都開発拠点で開催するイベントの軽食費などに当てようと思います。
...
以下のような意見もあり、主張されている内容に対して同意できる点もありますが、本記事は生成AI（LLM）を使って調査、裏付けは人間が行う形で執筆しています。
...
- Go 1.26 では `image/jpeg` の中身（エンコーダ＆デコーダ）がガッツリ入れ替わっている
- API はほぼそのまま、でもEncoder/Decoderは別物に近い
- Goを使ってサムネイルを生成している場合、ビット単位での互換性が壊れている可能性があるため、リグレッションテストが失敗するかも。注意。

### Go 1.26 変更点まとめ

「ざっくりまとめではわからない！」という方に向けたもう少し詳しく書いたものを掲載しておきます。
大きく以下のような点が更新される予定です。
（以下の内容は生成AIによってまとめてもらったものに手を加えています。そのため、誤った情報が含まれている、変更時に意図が変わってしまっている可能性があります）

- エンコーダ/デコーダ実装を置き換えることで、より高速・より正確な実装に変更
- パフォーマンスの向上
	- 現状でも十分速いが、特に大きな `jpeg` や複雑な画像ではボトルネックになりうるケースがあったのを改善
	- `decode` 側で約1割強の速度改善が見込まれるパッチが入る（[647835](https://go-review.googlesource.com/c/go/+/647835)）
- 数値的な「正確さ」
...
- 壊れた `jpeg` への耐性
	...
	- `restart marker` まわりの挙動を強化し、「`panic` ではなくエラーを返す」方向の修正が議論されている
		- [[go] image/jpeg: improve handling of JPEG restart markers in non-ideal cases](https://groups.google.com/g/golang-codereviews/c/D4XMjK5aDAE)
- 互換性
...
これまで `image/jpeg` は2009年頃から実装されてきたEncoder/Decoderをパフォーマンス改善やロバストネス改善など、少しずつパッチを当てる形で改善、対応してきました。
今回の変更ではいくつかの改善が含まれています。
...
これらの処理を事前計算（pre compute）にすることで、内側のループは係数の読み取りに遷延することができるようになるということのようです。
（とても難しい……）

内側のループの中で参照テーブルを、外側のループで `ponter` を固定しておくことで以下の効果が期待できます。

* 分岐回数削減
* CPUキャッシュヒット率向上
* ポインタ計算の削減

これらの処理はハフマンデコードの最も重い計算部分なので、ここが改善されることは全体的なデコード処理の高速化につながります。
...
これまでは 壊れた JPEG を読んだときに、`FormatError` や `io.ErrUnexpectedEOF` ではなく `panic` を起こしてしまうケースがあったようです。
...
Go 1.26ではLuaの `libjpeg` の「現実世界の壊れた JPEG に対するロバストさ」を参考に、`restart marker` の前にある余計なバイト（`spurious data`）をスキップしたり、予期しないマーカーが来ても なるべく復旧して`decode`を続行するロジックが入ったようです。
...
`jpeg` 規格的では「どのHuffmanコードを使うか」「どの順番で書くか」はかなり自由度が高くなっています。
そのため、どちらも規格には適合し、見た目の画素値も（デコーダ実装によっては）ほぼ同じだがバイト列はまったく一致しない、というのは現象が発生し得ます。
...
もし、`jpeg` まわりで バイト列一致テスト をしている場合、Go 1.26 に上げると落ちる可能性があるので「画素値レベル比較」もしくは、「許容誤差付き比較」に変えるなどの大作を行うほうが良いでしょう。
...
普段、プロジェクトマネージャーを行っているとこういったコンピューターサイエンスの分野を学ぶ機会は意識的に取り組まなければ中々実践できません。
今回はメジャーバージョンの大まかな機能の紹介ではなく、あえて1つのパッケージの変更にフォーカスし、深く掘り下げる形の記事を書いてみると自分自身の勉強にもなってよいのではないか？という思いつきからスタートしましたが、想像以上に楽しくコードを読むことができました。
